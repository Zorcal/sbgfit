...
func TestBuildExerciseFilter(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			queryParams: "?name=deadlift&category=strength&equipmentTypes=barbell&primaryMuscles=back,hamstrings&tags=functional,strength-endurance",
			want: mdl.ExerciseFilter{
		...
		},
		{
			...
			queryParams: "?name=&category=&equipmentTypes=&primaryMuscles=&tags=",
			want:        mdl.ExerciseFilter{},
		},
	}
	for _, tt := range tests {
